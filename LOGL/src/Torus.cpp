	...
	for (int i = 0; i < prec + 1; i++) {
		for (int ring = 1; ring < prec + 1; ring++) {
			for (int vert = 0; vert < prec + 1; vert++) {
				...
				// compute the texture coordinates for the vertices in the new rings (IMPORTANT!! - The below two lines are the only which are responsible for finding texture coordinates, { (ring * 1.0f) seems to be more pleasant } but book says to multiply 2.0f, so if they mess up in the output, you know where the culprit lies (just in the two lines below)
				texCoords[ring * (prec + 1) + vert] = glm::vec2((float)ring * 1.0f / (float)prec, texCoords[vert].t);
